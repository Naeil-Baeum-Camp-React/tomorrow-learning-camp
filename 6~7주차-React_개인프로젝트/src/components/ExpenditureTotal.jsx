...
function ExpenditureTotal({ expenditureTotalMap, allCategoryTotalPrice }) {
  console.log('ExpenditureTotalComponent');
  ...
  for (const key of expenditureTotalMap.keys()) {
    ...
    // 스타일 컴포넌트 push
    graphBarJsxList.push(<GraphBar key={color} $ratio={ratio} $color={color} />);
  }

  return (
    <Section>
      <TotalWrap>
        2월 총 지출: {allCategoryTotalPrice}원<TotalGraph>{graphBarJsxList}</TotalGraph>
